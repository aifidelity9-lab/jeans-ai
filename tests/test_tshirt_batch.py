"""Batch T-shirt try-on: all T-shirts x all models -> photos."""
...
async def run_single(garment: str, model: str, index: int, output_dir: str) -> dict:
    ...
    try:
        ...
        print(f"  [{index:02d}] OK  {g_name} x {m_name}  ({elapsed:.1f}s)", flush=True)
        return {"status": "ok", "file": result}
    except Exception as e:
        print(f"  [{index:02d}] FAIL {g_name} x {m_name}  ({e})", flush=True)
        return {"status": "fail", "error": str(e)}


async def main():
    products = get_images("assets/products/tshirts")
    models = get_images("assets/models/tshirts")

    if not products:
        print("No T-shirt images found in assets/products/tshirts/")
        print("Please add T-shirt product images first.")
        return

    print(f"T-shirts: {len(products)}")
    ...
    output_dir = "output/batch_tshirt"
